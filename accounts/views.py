    ...
    if request.method == 'POST':
        ...
        if report_type == 'academic':
            ...
            if behaviors:
                ...
                try:
                    highest_behavior = max(behaviors, key=lambda b: severity_map.get(b.severity.lower() if b.severity else '', 0))
                    highest_severity = highest_behavior.severity if highest_behavior.severity else "N/A"
                except Exception:
                    highest_severity = behaviors[0].severity if behaviors[0].severity else "N/A"
                
                try:
                    latest_behavior = max(behaviors, key=lambda b: b.date if b.date else datetime.min.date())
                    latest_date = latest_behavior.date if latest_behavior.date else "N/A"
                except Exception:
                    latest_date = "N/A"
